...
const [showChat, setShowChat] = useState(false);
  useEffect(() => {
    // Check if the appointment is in progress (status 1) or ended (status 2)
    if (apptStatus === 1) {
      setShowChat(true);
    } else {
      setShowChat(false);
  ...
  useEffect(() => {
    const fetchAppointment = async () => {
      try {
        ...
        setApptStatus(data.appt_status);
  ...
  try {
    const res = await fetch(`${apiUrl}/appt-status/${appointmentId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({appt_status: 1 }),
    });
    if (res.ok) {
      setApptStatus(1);
      setShowChat(true);
      setAppointmentData(prev => ({ ...prev, appt_status: 1 }));
      showSnack("Appointment started", "success");
    } else {
      showSnack("Failed to start appointment");
    }
  } catch (err) {
    console.error(err);
    showSnack("Error starting appointment");
  }
};
...
const endAppointment = async () => {
  try {
    const res = await fetch(`${apiUrl}/appt-status/${appointmentId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({appt_status: 2 }),
    });
    if (res.ok) {
      setApptStatus(2); // Change status to 2 (ended)
      setShowChat(false);
      setAppointmentData(prev => ({ ...prev, appt_status: 2 }));
      showSnack("Appointment ended", "success");
    } else {
      showSnack("Failed to end appointment");
    }
  } catch (err) {
    console.error(err);
    showSnack("Error ending appointment");
  }
};
          ...
          <Panel elevation={3}>
            ...
            {appointmentData ? (
              <>
                ...
                <Typography sx={{ fontSize: "1.3em", display: 'flex' }}>
                  ...
                  >
                    <Box sx={style}>
                      ...
                      <Box>
                        <Typography id="modal-modal-title" variant="h6" component="h2" color="black">Assign Meal Plan</Typography>
                        <div className='labels'>
                ...
                <Box mt={2}>
                  {appointmentData.appt_status === 0 && (
                    <Button sx={buttonStyle} onClick={startAppointment}>Start Appointment</Button>
                  )}
                  {appointmentData.appt_status === 1 && (
                    <Button sx={{ ...buttonStyle, backgroundColor: "#e57373" }} onClick={endAppointment}>
                      End Appointment
                    </Button>
                  )}
                </Box>
